...
function NotesFeed({
  ...
  // Build date list: last 7 days by default, or last 90 days if showAll
  const count = showAll ? 90 : 7;
...
function DiaryDayCard({
  ...
  return (
    <div className="mb-1">
      ...
      {/* Body */}
      <div className="bg-white dark:bg-neutral-900 border border-t-0 border-neutral-200/70 dark:border-neutral-800 rounded-b-xl px-5 pt-4 pb-5 mb-4">
        {/* Existing notes */}
        {notes.map((note) => (
          <p
            key={note.id}
            className="text-neutral-800 dark:text-neutral-200 text-[15px] leading-relaxed mb-2 pl-1 border-l-2 border-neutral-200 dark:border-neutral-700"
          >
            {note.title}
          </p>
        ))}
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-lg bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-neutral-100 dark:border-neutral-800 overflow-hidden"
      >
        {/* Date header */}
        <div className="bg-[#EDE9DF] dark:bg-neutral-800 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="text-xs bg-white/70 dark:bg-neutral-700 border border-neutral-300 dark:border-neutral-600 rounded-lg px-2 py-1.5 outline-none focus:ring-2 focus:ring-indigo-400 dark:text-white"
            />
        ...
        <div className="p-6">
          ...
          <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-neutral-100 dark:border-neutral-800">
            ...
            <button onClick={handleSave} disabled={saving || !text.trim()}
              className="flex items-center gap-2 px-5 py-2 rounded-xl bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 font-semibold text-sm hover:bg-neutral-700 dark:hover:bg-neutral-100 transition disabled:opacity-50">
              {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Plus className="w-3.5 h-3.5" />}
              Add Note
            </button>
          </div>
